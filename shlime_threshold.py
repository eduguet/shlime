...
def experiment_main(X, y):
    ...
    for n_estimators in [1,2,4,8,16,32,64]:
        for max_depth in [1,2,4,8,None]:
            for min_samples_split in [2,4,8,16,32,64]:
                ...
                # Train both LIME and SHAP models
                ...
                # Setup explainers
                lime_explainer = lime.lime_tabular.LimeTabularExplainer(xtrain, 
                                                                       feature_names=adv_lime.get_column_names(), 
                                                                       discretize_continuous=False)
                background_distribution = shap.kmeans(xtrain,10)
                shap_explainer = shap.KernelExplainer(adv_shap.predict, background_distribution)
                
                 # Get explanations for test set
                formatted_explanations = []
                for i in range(min(xtest.shape[0], 100)):
                    # Get LIME explanations
                    lime_exp = lime_explainer.explain_instance(xtest[i], adv_lime.predict_proba).as_list()
                    lime_dict = {feature: value for feature, value in lime_exp}
                    
                    # Get SHAP explanations
                    shap_values = shap_explainer.shap_values(xtest[i:i+1])[0]
                    shap_dict = {features[j]: shap_values[j] for j in range(len(features))}
                    
                    # Combine explanations by multiplication
                    combined_exp = []
                    for feature in features:
                        lime_value = lime_dict.get(feature, 0)
                        shap_value = shap_dict.get(feature, 0)
                        combined_value = lime_value * shap_value
                        combined_exp.append((feature, combined_value))
                    
                    formatted_explanations.append(combined_exp)
                ...
                # Default to using LIME's training task
    ...
    df.to_csv('data/threshold_results_combined.csv')
